...
export default function ReportsPage() {
  ...
  const [savedBrief, setSavedBrief] = useState<{ id: string; content: string; ai_model: string; title: string; updated_at: string } | null>(null)
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      const data = await res.json()
      const newSummary = data.summary || 'Failed to generate summary.'
      const newModel = data.model || ''
      setSummary(newSummary)
      setSummaryModel(newModel)
      setEditing(false)
      // Auto-save to briefs
      const exerciseTitle = exerciseId === 'all' ? 'All Exercises' : exercises.find(e => e.id === exerciseId)?.title || 'Report'
      const saveRes = await fetch('/api/admin/reports/briefs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        id: briefId, exercise_id: exerciseId === 'all' ? null : exerciseId || null,
        title: `${exerciseTitle} — Governance Brief`, content: newSummary, ai_model: newModel,
      })})
      const saveData = await saveRes.json()
      if (saveData.id) { setBriefId(saveData.id); setSavedBrief({ id: saveData.id, content: newSummary, ai_model: newModel, title: `${exerciseTitle} — Governance Brief`, updated_at: new Date().toISOString() }) }
  ...
  const copySummary = () => {
    navigator.clipboard.writeText(editing ? editContent : summary)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }

  const exportSummaryPdf = () => {
    const content = editing ? editContent : summary
    ...
    w.document.write(el?.querySelector('.prose')?.innerHTML || content)
  ...
  const saveBrief = async () => {
    ...
    const body = {
      id: briefId,
      exercise_id: exerciseId === 'all' ? null : exerciseId || null,
      title: `${exerciseTitle} — Governance Brief`,
      content: editContent,
      ai_model: summaryModel || null,
    }
    const res = await fetch('/api/admin/reports/briefs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    const data = await res.json()
    if (data.id) {
      setBriefId(data.id)
      setSavedBrief(prev => prev ? { ...prev, content: editContent, ai_model: summaryModel || prev.ai_model, title: body.title, updated_at: new Date().toISOString() } : null)
    }
  ...
  // Load existing brief when exercise changes
  useEffect(() => {
    if (!exerciseId) { setBriefId(null); setSavedBrief(null); return }
    const exId = exerciseId === 'all' ? null : exerciseId
    const params = exId ? `?exercise_id=${exId}` : ''
    fetch(`/api/admin/reports/briefs${params}`).then(r => r.json()).then(brief => {
      if (brief?.id) {
        setBriefId(brief.id)
        setSavedBrief(brief)
      } else {
        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          {loading ? (
            ...
            <div className="space-y-8 report-sections">
              ...
              {/* AI Summary */}
              {!summary && savedBrief && (
                <div className="rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 glass-card border-[#F3D59D]/20">
                  <div>
                    <p className="text-sm font-medium text-white/80">{savedBrief.title}</p>
                    <p className="text-xs text-white/40">Saved {new Date(savedBrief.updated_at).toLocaleDateString()} · {savedBrief.ai_model || 'AI'}</p>
                  </div>
                  <Button size="sm" onClick={() => { setSummary(savedBrief.content); setSummaryModel(savedBrief.ai_model || ''); setBriefId(savedBrief.id) }} className="bg-[#F3D59D] text-[#0b1326] hover:bg-[#e0c38c] text-xs whitespace-nowrap">
                    Load Saved Brief
                  </Button>
                </div>
              )}
                ...
                <section ref={summaryRef} className="rounded-2xl p-8 relative overflow-hidden" style={{ background: '#0F172A', border: '2px solid rgba(243,213,157,0.4)', boxShadow: '0 0 50px rgba(243,213,157,0.1)' }}>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => editing ? setEditing(false) : startEditing()} className="p-2 rounded border border-white/10 bg-white/5 hover:bg-white/10 transition-colors" title={editing ? 'Preview' : 'Edit Brief'}>
                        {editing
                          ? <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F3D59D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                          : <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F3D59D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        }
                      </button>
                      {editing && (
                        <button onClick={saveBrief} disabled={saving} className="p-2 rounded border border-[#F3D59D]/30 bg-[#F3D59D]/10 hover:bg-[#F3D59D]/20 transition-colors" title="Save Brief">
                          {saved
                            ...
                            : <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F3D59D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
                          }
                        </button>
                      )}
                  ...
                  {editing ? (
                    <textarea
                      ...
                      className="w-full min-h-[400px] bg-white/[0.03] border border-white/10 rounded-lg p-4 text-white/80 text-sm font-mono resize-y focus:outline-none focus:border-[#F3D59D]/40"
                      placeholder="Edit your governance brief..."
                    />
                  ) : (
